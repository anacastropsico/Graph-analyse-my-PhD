...
aibi_imuno_BI_2<- read_sav("D:/Ana Paula/Doutorado PPGNeC 2021-2024/2023/PAPER AIBI IEG'S/aibi imuno BI norm 2.sav") #edite pra o lugar que o arquivo est?
View(aibi_imuno_BI_2)

##CORRELA??O ##
summary(aibi_imuno_BI_2) #Algumas estatisticas bases da ?rea
matcorBI <- cor(aibi_imuno_BI_2, method = "spearman") #Faz a correla??o
print(matcorBI, digits = 2) #Exibe as correla??es
testRes = cor.mtest(aibi_imuno_BI_2, method = "spearman") #Colocar o n?vel de significancia pra as correa??es
...
#==================================================#
##Abrindo arquivo ##
BI <- read.csv("D:/Ana Paula/Doutorado PPGNeC 2021-2024/2023/PAPER AIBI IEG'S/R/dados/BI_2.txt", row.names=1, sep="")
View(BI) #Abrir arquivo
## Gerando a matriz##
require(igraph) #Puxa o pacote
links= as.matrix(BI) #Transforma o banco numa matriz pass?vel de ser lida

matrixnetwork2 = graph.adjacency(links, mode="undirected", weighted = TRUE, add.colnames=NULL, diag=FALSE)
plot(matrixnetwork2)
...
centr_betw(matrixnetwork,directed = FALSE, normalized = TRUE)
estimate_betweenness(matrixnetwork, directed = FALSE,  weights =TRUE,  cutoff = -1) #O vértice e a aresta intermediária são (aproximadamente) definidos pelo número de geodésicas (caminhos mais curtos) passando por um vértice ou aresta
...
global_efficiency(matrixnetwork, weights = NULL, directed = FALSE)
...
#plot final
plot(matrixnetwork, layout=layout_in_circle, vertex.size=degree(matrixnetwork) *6.2, 
...
BIbootdegree <- lapply(seq_len(Nperm), function(x){  
...
BIbootstrength <- lapply(seq_len(Nperm), function(x){  
  randomnet <- rewire(matrixnetwork, with=each_edge(0.5)) #rewire vertices with constant probability
  E(randomnet)$weight <- sample(E(matrixnetwork)$weight) #shuffle initial weights and assign them randomly to edges
  return(strength(randomnet))
})

BIbootcentr_Betw <- lapply(seq_len(Nperm), function(x){  
  randomnet <- rewire(matrixnetwork, with=each_edge(0.5)) #rewire vertices with constant probability
  E(randomnet)$weight <- sample(E(matrixnetwork)$weight) #shuffle initial weights and assign them randomly to edges
  return(centr_betw(randomnet)$res)
})

BIbootlocal_efficiency <- lapply(seq_len(Nperm), function(x){  
  randomnet <- rewire(matrixnetwork, with=each_edge(0.5)) #rewire vertices with constant probability
  E(randomnet)$weight <- sample(E(matrixnetwork)$weight) #shuffle initial weights and assign them randomly to edges
  return(local_efficiency(randomnet))
})

BIbootlocalaverage_eff <- lapply(seq_len(Nperm), function(x){  
  randomnet <- rewire(matrixnetwork, with=each_edge(0.5)) #rewire vertices with constant probability
  E(randomnet)$weight <- sample(E(matrixnetwork)$weight) #shuffle initial weights and assign them randomly to edges
  return(average_local_efficiency(randomnet))
})

BIbootglobal <- lapply(seq_len(Nperm), function(x){  
  randomnet <- rewire(matrixnetwork, with=each_edge(0.5)) #rewire vertices with constant probability
  E(randomnet)$weight <- sample(E(matrixnetwork)$weight) #shuffle initial weights and assign them randomly to edges
...
BIdegree_cool <- do.call("rbind", BIbootdegree)
BIstrength_coll <- do.call("rbind", BIbootstrength)
BIcentr_Betw_coll <- do.call("rbind", BIbootcentr_Betw) #ver como tirar apenas o $res boot_centr_Betw[[2]]$res
BIglobal_eff_cool <- do.call("rbind", BIbootglobal)
BIlocal_eff_cool <- do.call("rbind", BIbootlocal_efficiency)
BIlocalaverage_eff_cool <- do.call("rbind", BIbootlocalaverage_eff)
...
E(Randon_network)$weight <- sample(1:5, ecount(g), replace = TRUE)
plot(Randon_network)
...
###############################
###COMPARACOES ESTATASTICAS ###
###############################

###NORMALIDADE###

#NORMALIDADE REDE CONDICAO(AI OR BI)#
...
#Degree
t.test(BIdegree_cool[,1], Rdegree_cool[,1])
t.test(BIdegree_cool[,2], Rdegree_cool[,2])
t.test(BIdegree_cool[,3], Rdegree_cool[,3])
t.test(BIdegree_cool[,4], Rdegree_cool[,4])
t.test(BIdegree_cool[,5], Rdegree_cool[,5])
t.test(BIdegree_cool[,6], Rdegree_cool[,6])
t.test(BIdegree_cool[,7], Rdegree_cool[,7])

#Strength
t.test(BIstrength_coll[,1], Rstrength_coll[,1])
t.test(BIstrength_coll[,2], Rstrength_coll[,2])
t.test(BIstrength_coll[,3], Rstrength_coll[,3])
t.test(BIstrength_coll[,4], Rstrength_coll[,4])
t.test(BIstrength_coll[,5], Rstrength_coll[,5])
t.test(BIstrength_coll[,6], Rstrength_coll[,6])
t.test(BIstrength_coll[,7], Rstrength_coll[,7])

#Centrality #RESOLVER PROBLEMA
t.test(BIcentr_Betw_coll[,1], Rcentr_Betw_coll[,1])
t.test(BIcentr_Betw_coll[,2], Rcentr_Betw_coll[,2])
t.test(BIcentr_Betw_coll[,3], Rcentr_Betw_coll[,3])
t.test(BIcentr_Betw_coll[,4], Rcentr_Betw_coll[,4])
t.test(BIcentr_Betw_coll[,5], Rcentr_Betw_coll[,5])
t.test(BIcentr_Betw_coll[,6], Rcentr_Betw_coll[,6])
t.test(BIcentr_Betw_coll[,7], Rcentr_Betw_coll[,7])

#Global Efficiency #fazer wilcoxon pois valores s?o constantes
wilcox.test(global_eff_cool, RANboot_global_eff_cool)

#Local Efficiency
t.test(BIlocal_eff_cool[,1], Rlocal_eff_cool[,1])
t.test(BIlocal_eff_cool[,2], Rlocal_eff_cool[,2])
t.test(BIlocal_eff_cool[,3], Rlocal_eff_cool[,3])
t.test(BIlocal_eff_cool[,4], Rlocal_eff_cool[,4])
t.test(BIlocal_eff_cool[,5], Rlocal_eff_cool[,5])
t.test(BIlocal_eff_cool[,6], Rlocal_eff_cool[,6])
t.test(BIlocal_eff_cool[,7], Rlocal_eff_cool[,7])

#Local average Efficiency # fazer wilcoxon pois valore s?o constantes
t.test(BIlocalaverage_eff_cool, Rlocalaverage_eff_cool)